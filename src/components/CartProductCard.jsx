        ...
        <div key={item.productId} className="flex justify-between gap-2 mb-5">
            <div className="w-[70%] flex justify-between gap-2">
                <div className="flex flex-col gap-2">
                    <p className="text-xl">{item.product?.name ?? 'No Name'}</p>
                    <p className="font-bold">${item.product?.prices?.[0]?.amount ?? 0}</p>
            ...
            {/* product image */}
            <div className="w-[30%]">
                <img src={item.product?.gallery?.[0] ?? ''} className="w-full" alt={item.product?.name ?? 'Product'} />
            </div>
        </div>
    );
};
